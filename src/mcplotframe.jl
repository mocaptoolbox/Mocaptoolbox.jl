...
conncolor=:white)
    ...
    display(fig)
    return fig, ax, p, X, Y, Z, QW, QX, QY, QZ, txt, conn, pl
end
function plotframe(m::Mocapdata; framenum = 1,azimuth=0,elevation=0,showconn=true,showmnumbers=false,showmnames=false,showaxes=true,backgroundcolor=:white,figsize=(800, 600),msize=msize,mcolor=mcolor,mcolormap=colormap,connwidth=connwidth,conncolor=conncolor)
    ...
    rot = [GLMakie.Quaternion(QW[framenum,x],QX[framenum,x],QY[framenum,x],QZ[framenum,x]) for x in 1:m.nMarkers]
    p = meshscatter!(ax,X[framenum,:],Y[framenum,:],Z[framenum,:],markersize=msize,color=mcolor,colormap=mcolormap,clip_planes=Plane3f[],marker = GLMakie.Rect3f(Point3f(-1), Vec3f(2)),rotation=rot)
    if showconn == true
